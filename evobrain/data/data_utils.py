...
def getSeizureTimes(file_name):
    """
    Parses TUSZ binary seizure annotation. Supports both legacy v1.5.2
    `.tse_bi` (space-separated `start stop label prob` lines) and v2.0.6
    `.csv_bi` (CSV with `#` comment lines + header row, columns
    `channel,start_time,stop_time,label,confidence`).
    Args:
        file_name: edf file name (with or without .edf suffix)
    Returns:
        seizure_times: list of [start_sec, stop_sec] for label == 'seiz'
    """
    base = file_name.split(".edf")[0]
    csv_path = base + ".csv_bi"
    tse_path = base + ".tse_bi"
        ...
        with open(csv_path) as f:
            for line in f:
                ...
                if len(parts) < 4:
                    continue
                # channel,start_time,stop_time,label,confidence
                if parts[3].strip().lower() == "seiz":
                    seizure_times.append([float(parts[1]), float(parts[2])])
    else:
