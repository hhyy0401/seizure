...
FILEMARKER_DIR = _THIS.parent / 'file_markers_prediction'
    ...
    def _get_indiv_graphs(self, eeg_clip, swap_nodes=None):
        ...
        for i in range(0, num_sensors):
            for j in range(i + 1, num_sensors):
                ...
                xcorr = float(np.asarray(xcorr).squeeze())
                adj_mat[i, j] = xcorr
                adj_mat[j, i] = xcorr
    ...
    def __getitem__(self, idx):
        ...
        if self.graph_type == 'individual':
            ...
            if not _cache_ok:
                ...
                # Atomic write: tmp file + rename so partial files never appear at final path
                _tmp = cache_file_path + '.tmp.' + str(os.getpid())
                try:
                    with h5py.File(_tmp, 'w') as cache_file:
                        cache_file.create_dataset('supports', data=supports_seq.numpy())
                        cache_file.create_dataset('adj_mats', data=adj_mat_seq)
                    os.replace(_tmp, cache_file_path)
                except OSError:
                    try: os.remove(_tmp)
                    except OSError: pass
            ...
            # No-graph mode: ST/light models build their own structure.
            # Return tiny zero placeholders so default_collate / .to(device) works.
            supports_seq = torch.zeros(1, 1, 1, 1)
            adj_mat_seq = np.zeros((1, 1, 1), dtype=np.float32)
...
def load_dataset_prediction(
    ...
    if (graph_type is not None) and (
            graph_type not in ['individual', 'combined', 'dynamic', 'none']):
