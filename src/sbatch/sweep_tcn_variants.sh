#!/bin/bash
# Sweep: TCN-slot replacements (depthwise-sep 1D, TimesNet) × n_hyperedges {2,3,4}.
# All runs use --use_node_emb (learnable per-node embedding) and NO gamma feature.
# Runs sequentially on TUSZ v2.0.6, fix_threshold=0.5, sampling 1:1.
set -euo pipefail
...
MODELS="${MODELS:-light_st_hyper_dwsep light_st_hyper_timesnet}"
N_HYPEREDGES_LIST="${N_HYPEREDGES_LIST:-2 3 4}"
...
echo "=== TCN-variant sweep started: $stamp ==="
echo "Models:           $MODELS"
echo "n_hyperedges:     $N_HYPEREDGES_LIST"
echo "Epochs:           $NUM_EPOCHS  Patience: $PATIENCE  Eval every: $EVAL_EVERY"
echo "Node embedding:   ON   Gamma feature: OFF"
echo

for EH in $N_HYPEREDGES_LIST; do
    for MODEL in $MODELS; do
        SAVE_DIR="$RUNS_BASE/tusz_${MODEL}_NE_E${EH}_${CLIP_LEN}s_${stamp}"
        LOG="$REPO/logs/tusz_${MODEL}_NE_E${EH}_${stamp}.log"
